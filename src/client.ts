 ...
 * the SDK wire protocol over the child's stdio, fans server notifications out
 * to subscriptions, and tears the child down to quiescence through an
 * EOF → SIGTERM → SIGKILL ladder. Port of the in-repo
 * `@deepseek-ai/dsh-sdk-client` client; its design twin is the Python SDK's
 * `HarnessClient` — both drive the same runtime protocol.
 *
 * @module @openma/deepseek-harness-sdk/client
 */

import { spawn, type ChildProcess } from 'node:child_process'
import { disposeRuntimeProcess } from './dispose.js'
import { JsonRpcResponseError, RequestTimeoutError, SdkProtocolError, TransportClosedError } from './errors.js'
import type { ContentBlock, InitializeParams, InitializeResult, SessionPromptParams } from './protocol.js'
import { resolveDefaultLaunch } from './runtime.js'
import { JsonRpcLineTransport } from './transport.js'
import type { HarnessClientOptions, HarnessNotification, NotificationFilter } from './types.js'
 ...
 * EOF → SIGTERM → SIGKILL dispose ladder to quiescence. There is no
...
export class HarnessClient {
  ...
  start(): void {
    ...
    if (child.stdout === null || child.stdin === null) throw new TransportClosedError('runtime subprocess exposes no stdio pipes')
  ...
  /**
   * Send a fire-and-forget JSON-RPC notification to the runtime.
   * @param method - the wire method name.
   * @param params - the optional params object.
   */
  notify(method: string, params?: object): void {
    this.start()
    if (this.transport === undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      throw this.closedError('DeepSeek Harness runtime is not running')
    }
   ...
   * in its context; scoping is client-side). Ancestry discovered during any
   * subscription's lifetime is retained for the runtime process's lifetime.
   ...
   * bounded by `shutdownTimeoutMs`, then the stdin-EOF → SIGTERM → SIGKILL
   * ladder until the process actually exited. Idempotent and terminal.
  ...
  /**
   * Resolve the effective launch: an explicit `command` verbatim, else the
   * default resolution (`DSH_RUNTIME_BIN`, then the runtime npm package with
   * bundled-default-config injection when the caller set no non-empty
   * `DSH_CORDIS_CONFIG`).
   */
  private resolveLaunch(): { command: string; args: string[]; env: NodeJS.ProcessEnv } {
    ...
    if (this.options.command !== undefined) {
      return { command: this.options.command, args: this.options.args ?? [], env: base }
    }
    const resolved = resolveDefaultLaunch(base, this.options.runtimeResolveBase)
    const env = { ...base }
    const configured = env.DSH_CORDIS_CONFIG
    if (resolved.defaultConfigPath !== undefined && (configured === undefined || configured === '')) {
      // The runtime treats an empty value as absent, and so does this check.
      env.DSH_CORDIS_CONFIG = resolved.defaultConfigPath
    }
